...
public class PersonajeActivity extends AppCompatActivity {
            ...
            public void onResponse(Call<PersonajeDisney> call, Response<PersonajeDisney> response) {
                if (response.isSuccessful()){
                    if (response.code() == HttpURLConnection.HTTP_OK){
                        Toast.makeText(PersonajeActivity.this, "Funciona.", Toast.LENGTH_SHORT).show();
                        if (response.body() != null){
                            PersonajeDisney p = response.body();
                            lblNombre.setText(p.getName());
                            lblInfo.setText(hacerFrase(p));
    ...
    private String hacerFrase(PersonajeDisney p) {
        String frase;

        frase = "ID: " + p.getId() + "\n " +
                "Número de películas: " + p.getFilms().size() + "\n " +
                "Número de series: " + p.getTvShows();
